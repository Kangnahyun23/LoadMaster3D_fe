...
- **Trạng thái chuyển đổi:** `src/domain`, engine 3D và editor đã dùng cm (LM-031). Màn tài xế đọc revision đã duyệt
  bằng cm (LM-061). Màn kho và mock `LoadPlan` **vẫn là mm** cho tới LM-060/LM-062; viewer của kho đổi sang cm đúng một lần qua
...
- *(đã điều chỉnh, LM-030)* Planner đọc revision của chuyến qua `viewer-api.ts` → `usePlanSourceQuery` → `adaptResult → ViewerSceneModel` (cm, snapshot bất biến): revision đã duyệt mới nhất, hoặc `?revision=<jobId>`. `ScenePlacement` ghép `PackagePlacement` với kiện gốc (`packageId`, tên, điểm giao, `fragilityLevel`); `step = loadingOrder`. Tài xế (LM-061) đọc revision đã duyệt qua `driver-api.ts` → `useDriverPlanQuery` → `adaptResult`; kho còn `adaptLoadPlan` từ `LoadPlan` mm. Kết hợp `ViewerDraft` theo ID để sinh effective placements; chỉ commit `{ position?, orientation?, pinned? }`, vị trí draft là cm. Header hiện **MOCK RESULT** khi `isMockResult`. Chế độ màu thứ hai là **theo kiện gốc** (`packageId`) vì contract không có đơn hàng; `packaging` của kết quả là một kiểu trung tính.
