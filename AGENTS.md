...
- *(đã điều chỉnh, LM-036)* Loading lấy `placement.step` (= `loadingOrder`); unloading lấy `unloadingOrder` của kết quả qua `unloadSequence` (`operations/unloading.ts`), nhãn "Thứ tự dỡ" không kèm "gợi ý"; revision `ordersRecomputed` hiện thêm câu "tính lại ở FE". Chỉ phương án mm cũ không có `unloadingOrder` (tài xế) mới dùng thứ tự suy ra (stop tăng, cao trước, gần cửa trước) và giữ chữ "gợi ý". Stop-order consistency không chứng minh unload accessibility.
