...
try {
  ...
  await button('Trước').click(); await settle()
  ...
  await button('Góc chéo').click(); await settle()
  ...
  await settle()
  report.adaptation = { initial: (await metrics(page)).qualityTier }
  ...
  for (let i = 0; i < 85; i++) {
    ...
    if (i % 10 === 9 && (await metrics(page)).qualityTier === 'low') break
  }
