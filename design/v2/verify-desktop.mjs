...
try {
  ...
  await go('dashboard');await page.locator('#dashboard-period').selectOption('month');assert.equal(await page.locator('.kpi-tile strong').first().textContent(),'10');const download=page.waitForEvent('download');await page.locator('#export-summary').click();assert.match((await download).suggestedFilename(),/\.csv$/);
