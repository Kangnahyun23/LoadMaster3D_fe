...
test('nudge, history, pin, rotation, reset, focus and pointer drags on 1,000 packages', async ({ page, login, browserErrors }, testInfo) => {
  ...
  await page.getByRole('combobox', { name: 'Chọn kiện', exact: true }).selectOption('BENCH-01000')
  const initial = await position(page)
  await button(page, 'Tăng X').click()
  expect((await position(page))[0]).toBe(initial[0]! + 10)
  ...
  await button(page, 'R×D×C').click()
  expect(await status.innerText()).toMatch(/Không thể xoay/)
  expect(await status.getAttribute('data-orientation')).toBe('0')
  await button(page, 'C×R×D').click()
  expect(await status.getAttribute('data-orientation')).toBe('2')
  ...
  expect((await position(page))[2]).toBe(initial[2]! + 10)
  await button(page, 'Khôi phục mọi chỉnh sửa').click()
  ...
  const activeDrag = await drag(page, [-20, 0, 0], { measure: true })
...
test('touch editing uses 56px controls, a full-width scene and captured touch drags', { tag: ['@tablet', '@phone'] }, async ({ page, login, browserErrors }, testInfo) => {
  ...
  const initialX = await page.evaluate(async (url) => {
    const { createBenchmarkPlan } = (await import(url)) as typeof import('@/features/viewer3d/benchmark.mock')
    return createBenchmarkPlan(1000).placements.find((p) => p.id === 'BENCH-01000')!.position.x
  }, SOURCE_MODULES.benchmark)
  await enterEdit(page)
  await page.getByRole('combobox', { name: 'Chọn kiện', exact: true }).selectOption('BENCH-01000')
  const nudge = button(page, 'Tăng X')
  await nudge.tap()
  expect(await status.getAttribute('data-x')).toBe(String(initialX + 10))
  ...
  if (testInfo.project.name === 'tablet') {
    ...
    await button(page, 'Tập trung vào kiện').tap()
    await waitIdle(page)
    const start = await proxyPoint(page), end = await proxyPoint(page, [-20, 0, 0])
