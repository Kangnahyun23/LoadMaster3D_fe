...
test('loading, unloading advisories, blockers and approval wording on 1,000 packages', async ({ page, login, browserErrors }, testInfo) => {
  ...
  expect(await page.getByRole('dialog').innerText()).toMatch(/Thứ tự dỡ gợi ý/)
  await closeInspector(page)
  await button(page, 'Tiến một bước').click(); await settle(page)
  ...
  expect(await unloadSlider.inputValue(), 'advisory pauses the simulation').toBe(String(blockedIndex))
  cargo = await visibleCargo(page)
  ...
  expect(await blockerPanel.innerText()).toMatch(/có khả năng cản đường/)
