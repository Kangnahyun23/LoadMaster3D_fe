...
export function Timeline({ placements, step, totalSteps, playing, speed, onStepChange, onStepForward,
  ...
  return <div className="flex flex-none flex-wrap items-center gap-x-4 gap-y-1 border-t border-border bg-bg px-3 py-2 text-body-lg sm:flex-nowrap xl:px-5 xl:text-body" data-operation-timeline>
    <div className="flex shrink-0 items-center gap-1">
      <Button variant="secondary" className="size-14 p-0 xl:size-11" aria-label={t('viewer.timeline.start')} onClick={onGoToStart} disabled={!totalSteps}><SkipBack strokeWidth={1.5} /></Button>
      <Button variant="secondary" className="size-14 p-0 xl:size-11" aria-label={t('viewer.timeline.back')} onClick={onStepBackward} disabled={step <= minimum}><ChevronLeft strokeWidth={1.5} /></Button>
      <Button variant="secondary" className="size-14 p-0 xl:size-11" aria-label={playing ? t('viewer.timeline.pause') : t('viewer.timeline.play')} aria-pressed={playing} onClick={onTogglePlaying} disabled={!totalSteps}>
        {playing ? <Pause strokeWidth={1.5} /> : <Play strokeWidth={1.5} />}
      </Button>
      <Button variant="secondary" className="size-14 p-0 xl:size-11" aria-label={t('viewer.timeline.forward')} onClick={onStepForward} disabled={step >= totalSteps}><ChevronRight strokeWidth={1.5} /></Button>
    </div>
    <div className="ml-auto shrink-0 sm:ml-0"><span className="block text-text-2">{label}</span><span className="font-mono">{format.integer(step)} / {format.integer(totalSteps)}</span></div>
    <div className="order-last min-w-0 basis-full sm:order-none sm:flex-1 sm:basis-auto">
      <div className="mb-2 hidden justify-between gap-4 text-caption text-text-2 lg:flex">
        <span>{current ? t('viewer.timeline.current', { stop: current.stop, id: current.id }) : t('viewer.timeline.done')}</span>
    ...
    <label className="hidden shrink-0 flex-col text-caption text-text-2 sm:flex">{t('viewer.timeline.speed')}
