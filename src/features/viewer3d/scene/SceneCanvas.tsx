...
export function SceneCanvas({
  ...
  animateLoading = true, decoration = true, xraySelection, showMass, showDistribution, unloadMotion, onPerfSample, obstaclePicking = true, children,
}: SceneCanvasProps) {
  ...
  const obstacle = obstaclePicking ? model.vehicle.obstacles.find((o) => o.id === obstacleId) : undefined
  const handleSelect = useCallback((id: string | null) => { setObstacleId(null); onSelect(id) }, [onSelect])
  const handleObstacle = useCallback((id: string) => setObstacleId((current) => current === id ? null : id), [])
