...
export function EditorToolbar({ state, editor, onModeChange = editor.setMode }: {
  ...
  return (
    <div className="flex flex-none flex-wrap items-center gap-2 border-b border-border bg-bg px-2 py-1 sm:flex-nowrap" aria-label={t('viewer.editor.modeLabel')}>
      <div role="group" aria-label={t('viewer.editor.modeGroup')} className="flex gap-1">
        {(['view', 'edit'] as const).map((mode) => (
          <Button key={mode} variant="secondary" aria-pressed={editor.mode === mode} onClick={() => onModeChange(mode)}
            ...
            {t(mode === 'view' ? 'viewer.editor.view' : 'viewer.editor.edit')}
          </Button>
        ))}
      </div>
      {editor.mode === 'edit' ? <>
        <Button variant="ghost" aria-label={t('viewer.editor.undo')} disabled={!state.canUndo || dragging} onClick={editor.undo} className="size-14 p-0 xl:size-11">
          <Undo2 strokeWidth={1.5} />
        </Button>
        <Button variant="ghost" aria-label={t('viewer.editor.redo')} disabled={!state.canRedo || dragging} onClick={editor.redo} className="size-14 p-0 xl:size-11">
