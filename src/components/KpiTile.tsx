...
/**
 * Ô số liệu dùng chung (V2): icon trên nền tint, số lớn mono, nhãn, và một dòng ghi chú nói số đến từ đâu.
 *
...
export function KpiTile({
  ...
  return (
    <div role="group" aria-label={label} className="glass-tile flex min-h-21 items-start gap-3.5 rounded-lg px-4.5 py-3.5">
      {Icon ? (
        <span aria-hidden className={`grid size-10 flex-none place-items-center rounded-lg ${TONE_CLASS[tone]}`}>
          <Icon className="size-5" strokeWidth={1.5} />
        </span>
      ) : null}

      <div className="flex min-w-0 flex-col gap-1">
        <span className="font-mono text-[28px] leading-8 font-semibold tracking-[-0.02em] text-ink-strong tabular-nums">
          {value}
          {unit ? <span className="ml-1 font-mono text-body font-normal tracking-normal text-ink-2">{unit}</span> : null}
        </span>
        <span className="flex flex-wrap items-center gap-2 text-body text-ink-1">
          {label}
          {badge}
        </span>
        {note ? <span className="text-micro text-ink-2">{note}</span> : null}
      </div>
    </div>
  )
}
