...
- ~~Kiện không có trong bảng điểm giao: không được kiểm và không chắn kiện nào, không ném lỗi.~~ **Đổi khi gộp:** kiện được kiểm hoặc kiện chắn thiếu điểm giao → `throw` kèm mã kiện. Bỏ qua lặng lẽ sẽ giấu vi phạm LIFO khỏi bước chặn Duyệt, và mọi bảng tra khác của domain (khối lượng LM-021, thuộc tính xếp chồng LM-019, điểm giao LM-022) đều ném lỗi khi thiếu. Test tương ứng đổi thành kỳ vọng `throw` (đỏ trước khi sửa code).
...
**Gộp vào `feat/spec-mvp` (15/09/2026):** cherry-pick → `42d97ae`, xung đột duy nhất ở barrel `constraints/index.ts` (gộp với LM-019/LM-022). Sửa sau gộp: thiếu điểm giao → `throw` (xem quyết định ở trên). Tối ưu `candidates()` của lưới (lọc trước khi sắp) làm ở LM-023 cùng cổng benchmark.
